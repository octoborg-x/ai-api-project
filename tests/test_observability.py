...
def test_json_formatter_emits_structured_fields(caplog):
    ...
    with caplog.at_level(logging.INFO, logger=logger.name):
        logger.info("llm call", extra={"event":"llm.call","model":"test/model","tier":"balanced","latency_ms":1234.0,"input_tokens":500,"output_tokens":200,"total_tokens":700,"cost_usd":0.002,"status":"success"})
...
def test_record_call_does_not_log_prompt_contents(caplog):
    ...
    with caplog.at_level(logging.INFO, logger=logger.name):
        record_call(model="test/model", tier="cheap", prompt_tokens=10, completion_tokens=5, latency_ms=25.5, status="success")
